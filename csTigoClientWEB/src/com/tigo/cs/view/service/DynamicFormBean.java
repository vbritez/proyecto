...
@ViewScoped
public class DynamicFormBean extends AbstractCrudBeanClient<FeatureValue, FeatureValueFacade> {

	private static final long serialVersionUID = -466196929898797994L;
	public static final int COD_SERVICE = 20;
    ...
    public List<FeatureElement> getFeatureElementList() {
        if (featureElementList == null){
            featureElementList = featureElementFacade.getFeatureElementList(SecurityBean.getInstance().getLoggedInUserClient().getClient().getClientCod(), false);
        }
...
//                    List<Classification> classifications = classificationFacade.findByUserweb(SecurityBean.getInstance().getLoggedInUserClient());
                    ...
                    if (count == null) {
                        ...
                        // Se indica que se debe traer los de form dinamicos
                        where += " AND o.featureElement.clientFeature.feature.showOnMenuChr = false ";
...
//                            where = where.concat(" AND 1=2 ");
                ...
                public DataModel createPageDataModel() {
                    ...
                    if (classifications != null){
                        ...
                        where = where.concat(" AND 1=2 ");
                        return new ListDataModelViewCsTigo(getFacade().findRange(new int[] { getPageFirstItem(), getPageFirstItem() + getPageSize() }, where, null));
                    }
                    where = where.concat(getCabDetailWhereCriteria());
...
//        }
        ...
        if (selectedFeatureElement != null){
            where += " AND o.featureElement.featureElementCod = " + selectedFeatureElement.getFeatureElementCod();
        }
    ...
    public String getMessageDescriptionHeaderMap(FeatureValue fv) {
        ...
        if (typeOption == INTERNAL)
            messDescrip = fv.getMessage().getUserphone().getNameChr().concat(" | ").concat(sdf.format(fv.getMessage().getDateinDat()));
        else if (typeOption == EXTERNAL)
            messDescrip = fv.getPhone().getNameChr() != null ? fv.getPhone().getNameChr().concat(" | ").concat(sdf.format(fv.getMessage().getDateinDat())) : fv.getPhone().getCellphoneNum().toString().concat(" | ").concat(sdf.format(fv.getMessage().getDateinDat()));
        else{
