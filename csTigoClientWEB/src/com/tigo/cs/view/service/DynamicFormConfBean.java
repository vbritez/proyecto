...
import com.tigo.cs.facade.SendMessageFacade;
import com.tigo.cs.facade.UserphoneFacade;
import com.tigo.cs.security.SecurityBean;
import com.tigo.cs.view.administrative.CrudUserphoneBean;
 ...
 *          $
 */
@ManagedBean(name = "dynamicFormConfBean")
@ViewScoped
public class DynamicFormConfBean extends AbstractCrudBeanClient<FeatureElement, FeatureElementFacade> implements Serializable {
    ...
    @EJB
    private SendMessageFacade sendMessageProcess;
    ...
    private Boolean multiSelectOption = false;

    private boolean editing; // variable que indica si se puede o no editar el
                             // arbol de encuesta.

    // CAMBIO DE VIVI
    private boolean editItemList = false; // propiedad para saber si el nodo
                                          // seleccionado para editar es de
                                          // tipo
                                          // ITEM_LIST
    private boolean nodeBifurcate = false; // propiedad para saber si el nodo
                                           // seleccionado es de tipo
                                           // bifurcation
    private boolean bifurcation = false;// propiedad para saber si se esta
                                        // creando una bifurcacion
    ...
    private FeatureElement oldFeatureElemen;
    private Boolean treeUpdated = false;

    @PostConstruct
    public void init() {
        try {
            rootFeatureElementEntry = globalParameterFacade.findByCode("feature.dynamicform.ussdmenuentry.root.code");
            ussdApplicationServerId = globalParameterFacade.findByCode("feature.dynamicform.ussdapplicationserver.code");
            driverId = globalParameterFacade.findByCode("feature.dynamicform.ussddriver.code");
            validationId = globalParameterFacade.findByCode("feature.dynamicform.ussdvalidator.code");
            dynamicDriver = globalParameterFacade.findByCode("feature.dynamicform.ussddynamicdriver.code");
    ...
    private boolean validadedFields() {
        ...
        selectedFeature = featureFacade.find(Long.parseLong(featureDynamicFormId));

        selectedClientFeature = featureFacade.getClientFeature(
                SecurityBean.getInstance().getLoggedInUserClient().getClient().getClientCod(),
                selectedFeature.getFeatureCode());

        Integer count = featureElementFacade.getFeatureElementCount(
                SecurityBean.getInstance().getLoggedInUserClient().getClient().getClientCod(),
                selectedFeature.getFeatureCode());

        // SE VERIFICA QUE NO SE HAYA LLEGADO A LA MAXIMA CANTIDAD DE FEATURE
        // ELEMENTS CREADOS
        if (getEntity().getFeatureElementCod() == null
            && count >= selectedFeature.getMaxElementNum()) {
            setWarnMessage(i18n.iValue("web.client.screen.featureelement.message.ReachedMaxElement"));
            return false;
        }

        if (getEntity().getDescriptionChr() == null
            || getEntity().getDescriptionChr().equals("")) {
            setWarnMessage(i18n.iValue("web.client.screen.featureelement.message.DescriptionCanNotBeEmpty"));
            return false;
        }

        if (getEntity().getDescriptionChr() != null
            && getEntity().getDescriptionChr().length() > 50) {
        ...
        if (getEntity().getFeatureElementCod() == null
            && !startPeriodDateAfterToday(getEntity().getStartPeriodDat())) {
        ...
        if (selectedFeature != null) {
            ...
            if (selectedFeature != null
                && root.getChildren().get(0).getChildCount() > selectedFeature.getMaxEntryNum()) {
                setWarnMessage(MessageFormat.format(
                        i18n.iValue("web.client.screen.featureelement.message.EntriesMoreThanMaxValue"),
                        selectedFeature.getMaxEntryNum()));
                return false;
            }
        }
    ...
    public String saveEditing() {
        try {
            ...
            getEntity().setClientFeature(selectedClientFeature);
            getEntity().setMaxEntryNum(selectedFeature.getMaxEntryNum());
            getEntity().setMaxFeatureValue(selectedFeature.getMaxElementNum());
            getEntity().setClassifications(
                    selectedClassificationList != null ? selectedClassificationList : classificationList);
            getEntity().setFeatureElementEntries(Arrays.asList(getRootNode()));

            getEntity().setOpenChr(false);

            getEntity().setUserphones(
                    selectedUserphoneList != null ? selectedUserphoneList : userphoneList);
            getEntity().setPhoneLists(null);

            String retVal = featureElementFacade.saveEdit(getEntity(),
                    rootFeatureElementEntry, ussdApplicationServerId, driverId,
                    validationId, dynamicDriver);

            /*
             * Verificar si se hizo alguna modificacion, si se hizo, enviar sms
             * a los tel suscriptos por android para que actualice a la nueva
             * version
             */
            if (anyUpdateInDynamicForm()) {
                notifier.log(
                        getClass(),
                        "",
                        Action.INFO,
                        "SE ENVIARA MENSAJE A LOS USERPHONES QUE TENGAN ANDROID Y TENGAN ESTE FORMULARIO ACTIVO");
                sendAndroidUpdateMessage(getEntity().getUserphones());
            }
            ...
            setErrorMessage(MessageFormat.format(
                    i18n.iValue("web.admin.backingBean.commons.message.AnErrorHasOcurred"),
                    ex.getMessage()));
        } catch (Exception ex) {
            setErrorMessage(MessageFormat.format(
                    i18n.iValue("web.admin.backingBean.commons.message.AnErrorHasOcurred"),
    ...
    private void sendAndroidUpdateMessage(List<Userphone> userphoneList) {

        sendMessageProcess.sendMessageForUserphonesWithDynamicFormAndroid(getEntity().getFeatureElementCod());

        // insertIcon();
    }

    private Boolean anyUpdateInDynamicForm() {
        try {
            if (getEntity().getFeatureElementCod() == null
                || oldFeatureElemen == null)
                return false;
            if (userphoneListChanged(oldFeatureElemen.getUserphones(),
                    getEntity().getUserphones()))
                return true;
            if (!oldFeatureElemen.getDescriptionChr().equals(
                    getEntity().getDescriptionChr()))
                return true;
            if (oldFeatureElemen.getStartPeriodDat().compareTo(
                    getEntity().getStartPeriodDat()) != 0)
                return true;
            if (oldFeatureElemen.getFinishPeriodDat().compareTo(
                    getEntity().getFinishPeriodDat()) != 0)
                return true;
            if (treeUpdated)
                return true;
        } catch (Exception e) {
    ...
    private Boolean userphoneListChanged(List<Userphone> originalList, List<Userphone> newList) {
        if (originalList.size() != newList.size())
            return true;

        int count = 0;
        for (Userphone u1 : originalList) {
            for (Userphone u2 : newList) {
                if (u1.getUserphoneCod().longValue() == u2.getUserphoneCod().longValue()) {
                    count++;
                }
            }
        }
    ...
    private void getFeatureElementEntryListFromTree(TreeNode root) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if (entry.getFeatureElementEntries() != null
                && entry.getFeatureElementEntries().size() > 0) {
    ...
    public String editEntity() {
        ...
        /* Copiar datos a un nuevo objeto */
        try {
            oldFeatureElemen = (FeatureElement) getEntity().clone();
        ...
        /*
         * verificar si el feature element que se quiere editar ya tiene algun
         * dato cargado. Si ya tiene, no se pueden agregar/eliminar nodos al
         * arbol, solo editar nombre y descripcion. Sino, si se puede
         * agregar/eliminar/modificar el arbol entero.
         */
        List<FeatureValue> datas = featureValueFacade.getFeatureValueList(getEntity().getFeatureElementCod());
        if (datas == null || datas.size() == 0) {
            /*
             * editing= false significa que todavia no hay valores, y se puede
             * modificar el arbol
             */
            editing = false;
        } else {
        ...
        if (getFeatureElementEntriesList() != null
            && getFeatureElementEntriesList().size() > 0) {
            for (FeatureElementEntry fee : getFeatureElementEntriesList()) {
                fee.setIdTransient(fee.getFeatureElementEntryCod());
                featuresElementEntriesMap.put(fee.getFeatureElementEntryCod(),
                        fee);
            }
        }
        ...
        oldFeatureElemen.setUserphones(featureUserphoneList);
        ...
        if (getEntity().getOpenChr()) {
        ...
        } else if (featureUserphoneList != null
            ...
            if (phoneList != null && featurePhoneList.size() > 0
    ...
    public String deleteEntities() {
        ...
        try {
            featureElementFacade.deleteEntity(selectedEntities,
                    rootFeatureElementEntry);
    ...
    private void createEntryTree() {
        try {
            ...
            if (getEntity() != null
                && getEntity().getFeatureElementCod() != null) {
                ...
                getFeaturesElementEntriesMap().put(entry.getIdTransient(),
                        entry);
            }
    ...
    public String newChild() {
        ...
        /*
         * se verifica que el arbol pueda ser editable con la variable
         * onlyDescriptionEdition
         */
        ...
        /*
         * si se agrega hijo a una bifurcacion, se retorna directamente sin
         * validar nada
         */
        FeatureElementEntry currentEntry = (FeatureElementEntry) selectedNodes.getData();
        if (currentEntry.getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                BIFURCATION)) {
        ...
        /*
         * En caso que el nodo seleccionado sea un hijo del nodo raiz y no sea
         * una bifurcacion al cual si se le puede agregar hijos
         */
        if (selectedNodes.getParent().getParent() != null
            && selectedNodes.getParent().getParent().equals(root)
            && !currentEntry.getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                    BIFURCATION)) {
            setWarnMessage(i18n.iValue("web.client.screen.featureelement.message.CanNotAddChild"));
            datasValidated = false;
            RequestContext context = RequestContext.getCurrentInstance();
        ...
        /*
         * En caso que el nodo seleccionado sea un hijo de itemList tampoco se
         * les puede agregar mas hijos
         */
        if (selectedNodes.getParent().getParent() != null
            && currentEntry.getCodOwnerEntry().getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                    ITEM_LIST)) {
            setWarnMessage(i18n.iValue("web.client.screen.featureelement.message.CanNotAddChild"));
            datasValidated = false;
            RequestContext context = RequestContext.getCurrentInstance();
        ...
        /*
         * En caso que el nodo seleccionado sea un itemList y el padre un
         * option, tampoco se les puede agregar mas hijos
         */
        if (selectedNodes.getParent().getParent() != null
            && currentEntry.getCodOwnerEntry().getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                    OPTION)) {
        ...
        /*
         * si se esta editando, se debe traer los hijos del fee que se
         * selecciono
         */
        ...
        if (currentEntry.getFeatureElementEntries() != null) {
            /*
             * Si el nodo seleccionado no es de tipo bifurcacion al cual si se
             * le puede agregar hijos
             */
            /*
             * Recorremos los hijos del nodo seleccionado para verificar si ya
             * no tiene alguna bifurcacion, en este caso ya no se le podra
             * agregar hijos
             */
            if (!currentEntry.getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                    BIFURCATION)) {
                for (FeatureElementEntry fe : currentEntry.getFeatureElementEntries()) {
                    if (fe.getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                            BIFURCATION)) {
                        setWarnMessage(i18n.iValue("web.client.screen.featureelement.message.CanNotAddChild"));
                        datasValidated = false;
                        RequestContext context = RequestContext.getCurrentInstance();
                        context.addCallbackParam("datasValidated",
                                datasValidated);
                        return null;
                    }
                }
    ...
    public String editChild() {
        ...
        if (currentFeatElemEntry.getCodFeatureEntryType().getNameChr().equals(
                ITEM_LIST)
            || currentFeatElemEntry.getCodFeatureEntryType().getNameChr().equals(
                    BIFURCATION)
            || currentFeatElemEntry.getCodOwnerEntry() == null) {
            editItemList = true;
        }

        if (currentFeatElemEntry.getCodFeatureEntryType() != null
            && currentFeatElemEntry.getCodFeatureEntryType().getNameChr().equals(
                    OPTION)) {
            if (currentFeatElemEntry.getFeatureElementEntries() != null
                && currentFeatElemEntry.getFeatureElementEntries().size() >= 1) {
                option1 = currentFeatElemEntry.getFeatureElementEntries().get(0).getTitleChr();
            }
            if (currentFeatElemEntry.getFeatureElementEntries() != null
                && currentFeatElemEntry.getFeatureElementEntries().size() >= 2) {
                option2 = currentFeatElemEntry.getFeatureElementEntries().get(1).getTitleChr();
            }
            if (currentFeatElemEntry.getFeatureElementEntries() != null
                && currentFeatElemEntry.getFeatureElementEntries().size() >= 3) {
                option3 = currentFeatElemEntry.getFeatureElementEntries().get(2).getTitleChr();
            }
            if (currentFeatElemEntry.getFeatureElementEntries() != null
                && currentFeatElemEntry.getFeatureElementEntries().size() >= 4) {
                option4 = currentFeatElemEntry.getFeatureElementEntries().get(3).getTitleChr();
            }
            if (currentFeatElemEntry.getFeatureElementEntries() != null
                && currentFeatElemEntry.getFeatureElementEntries().size() >= 5) {
                option5 = currentFeatElemEntry.getFeatureElementEntries().get(4).getTitleChr();
            }
            multiSelectOption = currentFeatElemEntry.getMultiSelectOption();
            selectedTypeEntry = OPTION;
        } else if (currentFeatElemEntry.getCodFeatureEntryType() != null
            && currentFeatElemEntry.getCodFeatureEntryType().getNameChr().equals(
                    INPUT)) {
            selectedTypeEntry = INPUT;
        } else if (currentFeatElemEntry.getCodFeatureEntryType() != null
    ...
    public String deleteChild() {
        ...
        if (currentEntry.getFeatureElementEntries() != null) {
            for (int i = 0; i < currentEntry.getFeatureElementEntries().size(); i++) {
                getFeaturesElementEntriesMap().remove(
                        currentEntry.getFeatureElementEntries().get(i).getIdTransient());
            }
        }
            ...
            if (padre.getFeatureElementEntries() != null) {
                for (int i = 0; i < padre.getFeatureElementEntries().size(); i++) {
                    if (padre.getFeatureElementEntries().get(i).getIdTransient() == currentEntry.getIdTransient()) {
                        if (getFeaturesElementEntriesMap().get(
                                padre.getIdTransient()).getFeatureElementEntries() != null) {
                            getFeaturesElementEntriesMap().get(
                                    padre.getIdTransient()).getFeatureElementEntries().remove(
                                    i);
                        }
                    }
                }
        ...
        /*
         * se setea a null al padre del nodo seleccionado, y al nodo
         * seleccionado
         */
        ...
        /*
         * se crea un nuevo nodo en el arbol para el nodo que esta siendo
         * editado
         */
        nodoSeleccionado = new DefaultTreeNode(padre, parent);

        /*
         * se recorren a los hijos del nodo seleccionado y se referencia al
         * nuevo nodo padre
         */
        ...
        /*
         * SE CARGA LA LISTA DE HIJOS DEL PADRE DEL SELECCIONADO PARA PODER
         * DESFERENCIAR
         */
        ...
        /*
         * SE PONE SU PADRE A NULL Y SE GUARDAN LOS VALORES DEL ORDEN EN UN
         * ARRAY
         */
        ...
        /*
         * SE VUELVE A SETEAR LOS HIJOS AL PADRE, PERO EN ORDEN SEGUN EL
         * ORDERNUM
         */
        for (Integer i : orderNumList) {
            for (int j = 0; j < unorderedNodeList.size(); j++) {
                FeatureElementEntry node = (FeatureElementEntry) unorderedNodeList.get(
                        j).getData();
    ...
    public String saveElementEntry() {
        ...
        boolean datasValidated = true;
        if (currentFeatElemEntry.getTitleChr() == null
            || currentFeatElemEntry.getTitleChr().equals("")) {
        ...
        if (currentFeatElemEntry.getDescriptionChr() == null
            || currentFeatElemEntry.getDescriptionChr().equals("")) {
        ...
        if (currentFeatElemEntry.getTitleChr() != null
            && currentFeatElemEntry.getTitleChr().length() > 50) {
        ...
        if ((selectedTypeEntry != null && selectedTypeEntry.equals(OPTION))
            && ((option1 == null || option1.equals(""))
                ...
                && (option4 == null || option4.equals("")) && (option5 == null || option5.equals("")))) {
            datasValidated = false;
            RequestContext context = RequestContext.getCurrentInstance();
            context.addCallbackParam("datasValidated", datasValidated);
        ...
        if ((selectedTypeEntry != null && selectedTypeEntry.equals(OPTION))
            && ((option1 != null && !option1.equals("") && option1.length() > 50)
                ...
                || (option4 != null && !option1.equals("") && option4.length() > 50) || (option5 != null
                && !option1.equals("") && option5.length() > 50))) {
            datasValidated = false;
            RequestContext context = RequestContext.getCurrentInstance();
            context.addCallbackParam("datasValidated", datasValidated);
            setWarnMessage(i18n.iValue("web.client.screen.featureelement.message.MustSelectAtLeastOneOption"));
        ...
        if (selectedTypeEntry != null && selectedTypeEntry.equals(INPUT)) {
            ...
            /*
             * Si se esta editando un nodo y ahora es un INPUT, NO debe tener
             * hijos
             */
            if (currentFeatElemEntry.getFeatureElementEntries() != null) {
        ...
        if (selectedTypeEntry != null && selectedTypeEntry.equals(OPTION)) {
            ...
            currentFeatElemEntry.setMultiSelectOption(multiSelectOption);
            ...
            if (!onlyDescriptionEdition) {
                ...
                for (int i = 0; i < optionList.size(); i++) {
                    ...
                    getFeaturesElementEntriesMap().put(
                            optionEntry.getIdTransient(), optionEntry);
                }
        ...
        if (newChild || newChildBifurcation) {
            /*
             * Al que se esta creando le seteamos como padre el nodo
             * seleccionado y un id temporal
             */
        ...
        if (newChild || newChildBifurcation) {
            // Recuperamos del hashmap el nodo seleccionado al cual se le esta
            // agregando un nuevo hijo
            FeatureElementEntry aux = getFeaturesElementEntriesMap().get(
                    fe.getIdTransient());
            if (aux.getFeatureElementEntries() != null) {
                ...
                getFeaturesElementEntriesMap().put(
                        currentFeatElemEntry.getIdTransient(),
            ...
            /*
             * Si el padre del nodo que se esta editando no es nulo, es decir no
             * es la raiz
             */
            if (currentFeatElemEntry.getCodOwnerEntry() != null) {
                // Recuperamos del hashmap el padre del nodo seleccionado que se
                // esta editando
                FeatureElementEntry padre = getFeaturesElementEntriesMap().get(
                        fe.getCodOwnerEntry().getIdTransient());
                // Recorremos los hijos del padre del nodo seleccionado para
                // recuperar el que se edito
                for (int k = 0; k < padre.getFeatureElementEntries().size(); k++) {
                    if (equals(padre.getFeatureElementEntries().get(k).getIdTransient(),fe.getIdTransient())) {
                        ...
                        padre.getFeatureElementEntries().remove(k);
                        ...
                        // agregamos de vuelta al hashmap el padre ya con su
                        // hijo editado
                ...
                /*
                 * Recorremos todos los hijos del nodo seleccionado para cambiar
                 * la referencia del papa de los mismos al actual
                 */
                if (currentFeatElemEntry.getFeatureElementEntries() != null) {
                    for (int k = 0; k < currentFeatElemEntry.getFeatureElementEntries().size(); k++) {
                        // Recuperamos el hijo k y le seteamos como padre al
                        // actual
                        getFeaturesElementEntriesMap().get(
                                currentFeatElemEntry.getIdTransient()).getFeatureElementEntries().get(
                                k).setCodOwnerEntry(currentFeatElemEntry);
                    }
                }
                ...
                if (fe.getFeatureElementEntries() != null) {
                    for (int k = 0; k < fe.getFeatureElementEntries().size(); k++) {
                        // Recuperamos el hijo k y le seteamos como padre al
                        // actual
                        FeatureElementEntry hijok = getFeaturesElementEntriesMap().get(
                                fe.getIdTransient()).getFeatureElementEntries().get(
                                k);
                        getFeaturesElementEntriesMap().get(fe.getIdTransient()).getFeatureElementEntries().get(
                                k).setCodOwnerEntry(currentFeatElemEntry);
                        currentFeatElemEntry.getFeatureElementEntries().get(k).setIdTransient(
                                hijok.getFeatureElementEntryCod());
                    }
                }
                // borramos del hashmap el nodo que se edito y luego lo ponemos
                // ya con los cambios realizados
        ...
        if (newChild || newChildBifurcation) {
            ...
            /*
             * se crea un nuevo nodo para el nodo que se esta editando y se
             * referencia de vuelta
             */
            ...
            /* se setea a null all nodo seleccionado */
            // selectedNodes.setParent(null);
            selectedNodes = null;
        ...
        /* para indicar que hubo alguna modifiacion en el arbol del formulario */
        treeUpdated = true;
        return null;
    }
    ...
    public List<Feature> getFeatureList() {
        if (featureList == null) {
            /*
             * se trae todos los feature qeu tienen valor dynamicForm = true,
             * que son los que se usan para DynamicForm
             */
            featureList = featureFacade.getFeatureByClientByShowOnMenu(
                    SecurityBean.getInstance().getLoggedInUserClient().getClient().getClientCod(),
                    false);
        }
        return featureList;
    ...
    public List<PhoneList> getPhoneList() {
        if (phoneList == null) {
            try {
                phoneList = phoneListFacade.getPhoneListByClientType(
                        SecurityBean.getInstance().getLoggedInUserClient().getClient().getClientCod(),
                        WHITELIST);
    ...
    public List<PhoneList> getBlackPhoneList() {
        if (blackPhoneList == null) {
            try {
                blackPhoneList = phoneListFacade.getPhoneListByClientType(
                        SecurityBean.getInstance().getLoggedInUserClient().getClient().getClientCod(),
                        BLACKLIST);
            } catch (MoreThanOneResultException e) {
            } catch (GenericFacadeException e) {
    ...
    /*
     * Retorna la lista de clasificaciÃ³n o clasificaciones asociadas al usuario
     * logueado
     */
    ...
    public List<FeatureElementEntry> getFeatureElementEntryList() {
        ...
        FeatureElementEntry title = (FeatureElementEntry) root.getChildren().get(
                0).getData();
        ...
        if (paginationHelper == null) {
            ...
            paginationHelper = new PaginationHelper(pageSize) {
                ...
                public int getItemsCount() {
                    ...
                    where = where.concat(" AND o.clientFeature.feature.showOnMenuChr = false ");
    ...
    public String newBifurcation() {
        ...
        FeatureElementEntry currentEntry = (FeatureElementEntry) selectedNodes.getData();
        /*
         * En caso que no sea el titulo principal al cual siempre se le puede
         * agregar BIFURCACION y el nodo seleccionado no sea de tipo ITEM LIST
         * al cual tambien se le puede agregar BIFURCACION
         */
        if (!selectedNodes.getParent().equals(root)
            && !currentEntry.getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                    ITEM_LIST)) {
        ...
        /*
         * En caso que el seleccionado sea itemlist pero su padre sea option, no
         * se puede agregar bifurcacion
         */
        if (!selectedNodes.getParent().equals(root)
            && currentEntry.getCodOwnerEntry().getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                    OPTION)) {
    ...
    public String saveBifurcationElementEntry() {
        ...
        /*
         * Recuperamos del hashmap el nodo seleccionado y le agregamos un nuevo
         * hijo, luego lo borramos del hashmap y lo volvemos a insertar ya con
         * los cambios realizados
         */

        if (fe.getFeatureElementEntries() != null) {
            getFeaturesElementEntriesMap().get(fe.getIdTransient()).getFeatureElementEntries().add(
                    currentFeatElemEntry);
            getFeaturesElementEntriesMap().put(
                    currentFeatElemEntry.getIdTransient(), currentFeatElemEntry);
        } else {
            List<FeatureElementEntry> list = new ArrayList<FeatureElementEntry>();
            list.add(currentFeatElemEntry);
            getFeaturesElementEntriesMap().get(fe.getIdTransient()).setFeatureElementEntries(
                    list);
            getFeaturesElementEntriesMap().put(
                    currentFeatElemEntry.getIdTransient(), currentFeatElemEntry);
        }
        ...
        /* para indicar que se modifico el arbol del formulario */
        treeUpdated = true;
        return null;
    }
    ...
    private void createFeatureElementEntryTree() {
        try {
            // Recuperamos el FeatureElementEntry raiz para el FeatureElement
            // que se esta editando
            FeatureElementEntry raiz = featureElementEntryFacade.getRootFeatureElementEntryEager(getEntity());
            root = new DefaultTreeNode("root", null);

            /*
             * Creamos nuestro tree node pasandole como parametro la raiz y la
             * lista de featureElementEntry del featureElement que se esta
             * editando
             */
    ...
    private TreeNode createTree(FeatureElementEntry raiz, Collection<FeatureElementEntry> lista) {
        ...
        for (FeatureElementEntry hijo : hijos) {
            if (hijo.getFeatureElementEntries() != null
                && hijo.getFeatureElementEntries().size() > 0) {
                if (editing
                    || (!hijo.getCodFeatureEntryType().getNameChr().equalsIgnoreCase(
                            OPTION) && !editing)) {
    ...
    private List<FeatureElementEntry> getChildren(FeatureElementEntry nodo, Collection<FeatureElementEntry> lista) {
        for (FeatureElementEntry fe : lista) {
            if (nodo.getFeatureElementEntryCod() != null
                ...
                Collections.sort(fe.getFeatureElementEntries());
    ...
    /*
     * En caso que el featureElementEntriesList sea nulo, recuperamos por EAGER
     * toda la lista de FeatureElementEntry para el FeatureElement que se esta
     * editando
     */
    ...
    public void setFeatureElementEntriesList(List<FeatureElementEntry> featureElementEntriesList) {
    ...
    public void insertIcon() {
        List<IconType> iconType = iconTypeFacade.findIconEnabled();
        byte[] bytebuff = null;
        for (IconType icon : iconType) {
            try {
                ...
                IconType xx = iconTypeFacade.find(icon.getIconTypeCod());
                xx.setFileByt(bis.toByteArray());
                iconTypeFacade.edit(xx);
    ...
    public void handleFileUpload(FileUploadEvent event) {
        if (event.getFile() != null) {
            IconType xx = iconTypeFacade.find(101L);
            xx.setFileByt(event.getFile().getContents());
            iconTypeFacade.edit(xx);
        }
    }
}
